    ...
    def generate(
        ...
        if not self.client:
            ...
            return (
                ...
                if not stream
                else (yield "Error: Ollama client not available")
            )
        ...
        try:
            if stream:
                ...
                try:
                    ...
                    result = self._generate_with_subprocess(
    ...
    def _generate_with_subprocess(self, prompt, system_prompt, temperature, max_tokens):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                logger.error(
                    f"Ollama CLI command failed with code {result.returncode}: {result.stderr}"
                )
                return None
            ...
            try:
                ...
                for chunk in stream:
                    ...
                    if elapsed > max_wait_time:
                        logger.warning(
                            f"Streaming response terminated: timeout after {elapsed:.2f} seconds"
                        )
                        yield "\n[Response timed out]"
                        break
                    ...
                    if chunks_received > max_chunks:
                        logger.warning(
                            f"Streaming response terminated: too many chunks ({chunks_received})"
                        )
                        yield "\n[Response too long]"
                        break
                    ...
                    if data_silence > data_timeout:
                        logger.warning(
                            f"Streaming response terminated: no data for {data_silence:.2f} seconds"
                        )
                        yield "\n[Response stalled]"
                        break

                logger.debug(
                    f"Stream completed: received {chunks_received} chunks in {time.time() - start_time:.2f} seconds"
                )
