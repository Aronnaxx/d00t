...
class CLIController(cmd.Cmd):
    ...
    def default(self, line: str) -> bool:
        ...
        if self.central_model:
                    ...
                    try:
                        ...
                        for chunk in response:
                            ...
                            if time.time() - start_time > max_wait_time:
                                logger.warning(
                                    f"Response streaming timed out after {max_wait_time} seconds"
                                )
                                print("\n[Response timed out]")
                                break
                            ...
                            if time.time() - operation_start > max_total_time:
                                logger.warning(
                                    f"Total operation timed out after {max_total_time} seconds"
                                )
                                print("\n[Operation timed out]")
                                break
            ...
            print(
                "Natural language processing not available. Try basic commands like 'move forward'."
            )
            return True
        ...
        try:
            ...
            if model_name and model_name != self.central_model.model_name:
                ...
                temp_provider = LLMProviderFactory.create_provider(
                    provider_type=self.central_model.provider_type, model_name=model_name
                )
