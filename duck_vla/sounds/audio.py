...
class AudioSystem:
    ...
    def play_sound(self, filename: str) -> bool:
        ...
        try:
            ...
            if file_sample_rate != self.sample_rate:
                logger.debug(
                    f"Converting sample rate from {file_sample_rate} to {self.sample_rate}"
                )
            ...
            self.play_thread = threading.Thread(
                target=self._play_audio_thread, args=(data, play_sample_rate)
            )
    ...
    def start_recording(self, duration: Optional[float] = None) -> bool:
        ...
        logger.info(
            f"Starting audio recording" + (f" for {duration:.1f}s" if duration else " continuously")
        )
        ...
        try:
            ...
            self.record_thread = threading.Thread(
                target=self._record_audio_thread, args=(duration,)
            )
    ...
    def save_recording(self, filename: str) -> bool:
        ...
        try:
            ...
            # Write to file
            sf.write(filename, data, self.sample_rate, subtype="PCM_16")
